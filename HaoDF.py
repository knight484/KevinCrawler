...
class HaoDF:
    ...
    def get_query_info(self, url_list):
        ...
        for e, u in enumerate(url_list):
            ...
            #         query = dict()
            #         query['url'] = u
            # query['咨询地址'] = soup.find('div', class_='a_box').a['href']
            # query['咨询标题'] = soup.find('div', id='lidingbu').find('span', class_='fl').text.strip()
            #         query['标题'] = soup.find('div', id='lidingbu').find('span', class_='fl').text.strip()
            #         query['医生姓名'] = soup.find('h1', class_='doctor-name').text.strip()
            #         query['医师职称'] = re.sub(r' ', '', soup.find('span', class_='positon').text).strip()
            #         query['医师医院'] = soup.find('p', class_='doctor-faculty').text.split('\n')[1]
            ...
            #         query['医师主页'] = 'https:' + soup.find('a', class_='tab-item first')['href']
            #         try:
            #             query['患者状态'] = re.search(r'状态：(.+) \n', i.find('div', class_='stream_yh_left').text).group(1)
            #             query['身份'] = '患者'
            #         except AttributeError:
            #             query['身份'] = '医生'
            #         day = i.find('div', class_='yh_l_times').text.strip()
            #         if len(day) < 6:
            #             day = f'{dt.date.today().year}-{day}'
            #         elif day == '昨天':
            ...
            #             day = str()
            #         query['日期'] = day
            #         content = i.find('div', class_='stream_yh_right').find_all('div')
            #         query['内容'] = ''
            #         for c in content:
            #             if c.script:
            #                 c.script.clear()
            #             if c.text not in query['内容']:
            #                 query['内容'] += c.text.strip()
            #         query['内容'] = re.sub(r'发表于：\d{4}-\d{2}-\d{2}', '', query['内容'])
            #         query['uid'] = u.split('/')[-1].split('.')[0] + '_' + query['日期'].replace('-', '') + \
            #                        query['内容'][:10]
            #         # 保存数据
            #         table.update_one({'uid': query['uid']}, {"$set": query}, True)
            #         n += 1
            # else:
            #     items = soup.find_all('div', class_='f-card')
            #     if len(items) > 0:
            #         for i in items:
            #             query = dict()
            #             query['url'] = u
            #             # query['咨询地址'] = soup.find('div', class_='a_box').a['href']
            #             # query['咨询标题'] = soup.find('div', class_='fl-title ellps').text.strip()
            #             query['标题'] = soup.find('div', class_='fl-title ellps').text.strip()
            #             query['医生姓名'] = soup.find('h1', class_='doctor-name').text.strip()
            #             query['医师职称'] = re.sub(r' ', '', soup.find('span', class_='positon').text).strip()
            #             query['医师医院'] = soup.find('p', class_='doctor-faculty').text.split('\n')[1]
            #             query['医师科室'] = soup.find('p', class_='doctor-faculty').text.split('\n')[2]
            #             query['医师主页'] = 'https:' + soup.find('a', class_='tab-item first')['href']
            #             try:
            #                 query['患者状态'] = re.search(r'状态：(.+) \n', i.find('div', class_='f-c-l-s-green').text).group(
            #                     1)
            #                 query['身份'] = '患者'
            #             except AttributeError:
            #                 query['身份'] = '医生'
            #             query['日期'] = i.find('div', class_='f-c-l-date').text.strip().replace('.', '-')
            #             content = i.find('div', class_='f-c-right').find_all('div')
            #             query['内容'] = ''
            #             for c in content:
            #                 if c.script:
            #                     c.script.clear()
            ...
            #             query['uid'] = u.split('/')[-1].split('.')[0] + '_' + query['日期'].replace('-', '') + \
            #                            query['内容'][:10]
            #             if not query['日期'].startswith('20'):
            #                 if '.' in query['日期']:
            #                     query['日期'] = f'{dt.date.today().year}.{query["日期"]}'
            #                 else:
            #                     query['日期'] = str(dt.date.today()).replace('-', '.')
            #             # 保存数据
            #             table.update_one({'uid': query['uid']}, {"$set": query}, True)
            #             n += 1
