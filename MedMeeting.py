...
class MedMeeting:
    ...
    def get_doctor_link(self, keyword=''):
        # 初始化 ==========
        table = self.db['医生网址']
        ...
        max_pg = 100
        while cur_pg <= max_pg:
            u = f'https://www.medmeeting.org/Resource/speakersearch/?page={cur_pg}&name={keyword}'
            ...
            try:
                max_pg = int(re.search(r'(\d+)', soup.find('div', class_='show_page').find_all('a')[-1]['href']).group(1))
            ...
            spend = end - start
            unit_spend = spend / cur_pg
            remain = (int(max_pg) - cur_pg) * unit_spend
            print(f"\r进度({cur_pg}/{max_pg}), "
