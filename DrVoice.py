...
class DrVoice:
    ...
    def get_course_id(self):
        ...
        while n <= int(max_page):
            ...
            for item in items:
                ...
                course['post_link'] = item.find('img')['src']
                t1 = time.time()
    ...
    def get_doctor_info(self, n_list):
        table = self.db['医生信息']
        table.create_index('uid')

        start = time.time()
        for e, n in enumerate(n_list):
            url = f'https://www.drvoice.cn/user/{n}'
            r = req_get(url, header=self.header, proxy=self.proxy)
            soup = BeautifulSoup(r.text, 'lxml')
            doctor = dict()
            doctor['uid'] = n
            doctor['url'] = f'https://www.drvoice.cn/user/{n}'
            try:
                doctor['姓名'] = soup.find('div', class_="doctor-author").find('a').text.strip()
            except AttributeError:
                continue
            try:
                doctor['医院'] = soup.find('div', class_='doctor-hospital').text.strip()
            except AttributeError:
                pass
            try:
                doctor['简介'] = soup.find('div', class_="doctor-info").text.strip()
            except AttributeError:
                pass
            t1 = time.time()
            ...
            remain = (len(n_list) - (e + 1)) * unit_spend
            print(f"\r进度({(e + 1)}/{len(n_list)}), 本次数据存储用时{round(end - t1, 4)}秒"
                  f"用时{int(spend // 3600)}:{int(spend % 3600 // 60)}:{int(spend % 60)}, "
                  f"预计还剩{int(remain // 3600)}:{int(remain % 3600 // 60)}:{int(remain % 60)}.", end='')
